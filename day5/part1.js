...
const workSequence = (sequence) => {
    ...
    for(let i = 0; i < numberOfStrToMove; i++) {
        const sourceStack = stacks[sourceStrIndex];
        const string = sourceStack.slice(sourceStack.length - 1);
        const remainingSourceStack = sourceStack.slice(0, sourceStack.length - 1);
